...
import net.conselldemallorca.helium.ms.auth.ApiKeyAuth;
import net.conselldemallorca.helium.ms.auth.Authentication;
import net.conselldemallorca.helium.ms.auth.HttpBasicAuth;
import net.conselldemallorca.helium.ms.auth.OAuth;
...
public class ApiClient {
	...
	private String basePath = "http://pendent.de.definir.es";
	...
	protected void init() {
		...
		authentications.put("HttpBasicAuth", new HttpBasicAuth());
	...
	public <T> T invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body,
		...
		if (queryParams != null) {
			for (String param : queryParams.keySet())
				builder.queryParam(param, queryParams.get(param).toArray());
		}
		...
		HttpEntity<Object> requestEntity = new HttpEntity<Object>(selectBody(body, formParams, contentType), headers);

		ResponseEntity<T> responseEntity = restTemplate.exchange(builder.build().toUri(), method, requestEntity,
				returnType);
		...
		if (responseEntity.getStatusCode() == HttpStatus.NO_CONTENT) {
			return null;
		} else if (responseEntity.getStatusCode().series() == Series.SUCCESSFUL) {
			if (returnType == null) {
				return null;
			}

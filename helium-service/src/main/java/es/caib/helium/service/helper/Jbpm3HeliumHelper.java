...
import es.caib.helium.commons.dto.DominiRespostaColumnaDto;
import es.caib.helium.commons.dto.DominiRespostaFilaDto;
import es.caib.helium.commons.dto.EntornDto;
import es.caib.helium.commons.dto.EnumeracioValorDto;
...
import es.caib.helium.persistence.repository.CarrecRepository;
...
import es.caib.helium.persistence.repository.FestiuRepository;
...
import es.caib.helium.service.helper.TascaSegonPlaHelper.InfoSegonPla;
...
public class Jbpm3HeliumHelper implements Jbpm3HeliumService {
	...
	public ExpedientDto getExpedientAmbEntornITipusINumero(
		...
		logger.debug("Obtenint expedient donat entorn, tipus i número (" +
				...
				"numero=" + numero + ")");
		Entorn entorn = entornRepository.getReferenceById(entornId);
		if (entorn == null)
...
//				null).getId();
	...
	public ScspRespostaPinbal consultaPinbal(
		...
		Entorn entorn = entornRepository.getReferenceById(expedient.getEntorn().getId());
		if (entorn == null)
...
//			scspConfirmacioPeticioPinbal.getAtributos().setIdPeticion("PROVA_ASYNC");
	...
	public void expedientRelacionar(
		...
		Expedient origen = expedientRepository.getReferenceById(expedientIdOrigen);
		if (origen == null)
			throw new NoTrobatException(Expedient.class, expedientIdOrigen);
		Expedient desti = expedientRepository.getReferenceById(expedientIdDesti);
		if (desti == null)
	...
	public ExpedientDto findExpedientAmbMateixTipusINumero(
		...
		if (entorn == null)
			throw new NoTrobatException(Entorn.class, entornId);
		ExpedientTipus expedientTipus = expedientTipusRepository.getReferenceById(expedientTipusId);
		if (expedientTipus == null)
	...
	public List<ExpedientDto> findExpedientsConsultaGeneral(
		...
		if (expedientTipusId != null) {
			expedientTipus = expedientTipusRepository.getReferenceById(expedientTipusId);
			if (expedientTipus == null)
				throw new NoTrobatException(ExpedientTipus.class, expedientTipusId);
			if (estatId != null) {
				estat = estatRepository.getReferenceById(estatId);
				if (estat == null)
		...
		try {
			...
			for (InstanciaProcesDto instanciaProces: arbreProcessos) {
				...
				for (ExpedientDocumentDto document : documentsPerAfegir) {
					...
					DocumentStore documentStore = documentStoreRepository.getReferenceById(document.getId());

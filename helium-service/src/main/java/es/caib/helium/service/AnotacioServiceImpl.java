...
import es.caib.helium.service.helper.EmailHelper;
...
public class AnotacioServiceImpl implements AnotacioService, ArxiuPluginListener {
	...
	public AnotacioDto findAmbId(Long id) {
		logger.debug(
				...
				"id=" + id +  ")");
		Anotacio anotacio = anotacioRepository.findById(id).orElse(null);
	...
	public ArxiuDto getAnnexContingutVersioImprimible(Long annexId) {
		...
		// Recupera l'annex
		AnotacioAnnex annex = anotacioAnnexRepository.findById(annexId).orElse(null);
		if (annex == null)

...
//		}
	...
	public Tasca findTascaByJbpmTaskId(
			String jbpmTaskId) {
		return findTascaByJbpmTask(workflowEngineApi.getTaskById(jbpmTaskId));
	}
	...
	public List<ExpedientTascaDto> findTasquesPerExpedientPerInstanciaProces(
		...
		List<WTaskInstance> tasks = workflowEngineApi.findTaskInstancesForProcessInstance(processInstanceId);
...
//			WDelegationInfo delegationInfo = getDelegationInfo(task);
...
//			}
		...
		if (task.getActorId() != null) {
			dto.setResponsable(
					this.findPersonaOrDefault(task.getActorId()));
		}
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		if (task.getActorId() != null) {
			if (auth != null) {
	...
	public void createDelegationInfo(
			...
			String comentari,
			boolean supervisada) {
		WDelegationInfo info = null; //new WDelegationInfo();
	...
	public List<PersonaDto> findParticipants(Expedient expedient) {
		...
		for (WTaskInstance task: tasks) {
			if (task.getActorId() != null && ! codis.contains(task.getActorId())) {
				codis.add(task.getActorId());
				participants.add(this.findPersonaOrDefault(task.getActorId()));
			}
		}
		return participants;
	}

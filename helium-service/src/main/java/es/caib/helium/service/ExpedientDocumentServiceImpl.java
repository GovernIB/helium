...
import es.caib.helium.commons.dto.document.NtiDetallDto;
import es.caib.helium.commons.dto.document.PsignaDetallDto;
import es.caib.helium.commons.dto.document.RegistreDetallDto;
import es.caib.helium.commons.dto.document.SignaturaDetallDto;
import es.caib.helium.commons.dto.document.SignaturaValidacioDetallDto;
...
public class ExpedientDocumentServiceImpl implements ExpedientDocumentService {
	...
	public DadesNotificacioDto notificarDocument(
		...
		/* Oppció 1: codi actual corregit
		DocumentDto documentDto = documentHelperV3.toDocumentDto(
				documentStoreId,
				true,
				false,
				true, // ambContingutVista per obtenir el document firmat de custòdia #
				false,
				true, // Per notificar
				false);		
		...
		*/
		...
		if (arxiuDto.getContingut() == null) {
			DocumentStore documentStore = documentStoreRepository.findById(documentStoreId).orElse(null); //OBTENIR PER ID
		...
		Interessat interessatEntity	= interessatRepository.findById(interessatId).orElse(null);
		...
		if (representantId != null) {
			Interessat representantEntity = interessatRepository.findById(representantId).orElse(null);
    	...
    	if (ExpedientTipusTipusEnumDto.ESTAT.equals(expedient.getTipus().getTipus())) {
			...
			if (pendentsFirma!=null && pendentsFirma.size()>0) {
				...
				for (Portasignatures p: pendentsFirma) {
					...
					DocumentStore sdPf = documentStoreRepository.findById(p.getDocumentStoreId()).orElse(null);
...
//	    						aux.setArxiuEstat(arxiuEstat);
    ...
    private DocumentFinalitzarDto anotacioAnnexJaIncluit(AnotacioAnnex anotacioAnnex, List<DocumentFinalitzarDto> expedientDocuments) {
    	...
    	if (expedientDocuments!=null && expedientDocuments.size()>0) {
    		for (DocumentFinalitzarDto expedientDocumentDto: expedientDocuments) {
    			if (expedientDocumentDto.getAnnexAnotacioId()!=null && expedientDocumentDto.getAnnexAnotacioId().compareTo(anotacioAnnex.getId())==0) {
    				return expedientDocumentDto; //El annex del parametre 1, ja esta inclós als documents de la instancia de procés (2)
    			}
    		}
    	}
    ...
    private DocumentFinalitzarDto notificacioAnnexJaIncluit(DocumentStore notificacioAnnex, List<DocumentFinalitzarDto> expedientDocuments) {
    	
    	if (expedientDocuments!=null && expedientDocuments.size()>0) {
    		for (DocumentFinalitzarDto expedientDocumentDto: expedientDocuments) {
    			if (expedientDocumentDto.getDocumentStoreId().compareTo(notificacioAnnex.getId())==0) {
    				return expedientDocumentDto; //El annex del parametre 1, ja esta inclós als documents de la instancia de procés (2)
    			}
    		}
    	}
	...
	public DocumentDetallDto getDocumentDetalls(Long expedientId, Long documentStoreId) {
		...
		if (document.getAnotacioId() != null) {
			Anotacio anotacio = anotacioRepository.findById(document.getAnotacioId()).orElse(null);
			if (anotacio == null) {

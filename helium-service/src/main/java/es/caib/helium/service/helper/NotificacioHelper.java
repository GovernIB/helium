...
//		}
	...
	public DocumentNotificacio toDocumentNotificacio(DadesNotificacioDto dadesNotificacio) {
		...
		notificacio.setExpedient(expedientRepository.getReferenceById(dadesNotificacio.getExpedientId()));
		...
		notificacio.setDocument(documentStoreRepository.getReferenceById(dadesNotificacio.getDocumentId()));
		...
		if(dadesNotificacio.getDocumentsDinsZip()!=null && !dadesNotificacio.getDocumentsDinsZip().isEmpty()) {
			...
			for (DocumentStoreDto dsDto : dadesNotificacio.getDocumentsDinsZip()) {
				DocumentStore ds = documentStoreRepository.getReferenceById(dsDto.getId());
	...
	public DadesNotificacioDto toDadesNotificacioDto(DocumentNotificacio notificacio, boolean arxiuActiu) {
		...
		DocumentStore document = documentStoreRepository.getReferenceById(notificacio.getDocument().getId());
		...
		if (notificacio.getEnviamentCertificacio() != null) {
			DocumentStore justificant = documentStoreRepository.getReferenceById(notificacio.getEnviamentCertificacio().getId());
...
//		dadesNotificacio.setDocumentArxiuContingut(notificacio.getDocument().getArxiuContingut());
	...
	public void omplirModelNotificacioDto (DocumentNotificacioDto dto) {
		...
		if (dto.getEnviamentCertificacio() != null) {
			DocumentStore justificant = documentStoreRepository.getReferenceById(dto.getEnviamentCertificacio().getId());

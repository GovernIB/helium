...
//		} else
		if (isSignaturaFileAttached()) {
			List<byte[]> signatures = documentStore.getContinguts().stream().map(c -> c.getArxiuContingut()).collect(Collectors.toList());
...
//					documentStore.getReferenciaCustodia());
			List<byte[]> signatures = Lists.newArrayList(documentStore.getArxiuContingut());
	...
	public DocumentStore crearDocument(
		...
		if(annexosPerNotificar!=null && !annexosPerNotificar.isEmpty()) {
			...
			for(ExpedientDocumentDto exp : annexosPerNotificar) {
				DocumentStore documentStoreCont = documentStoreRepository.findById(exp.getId()).orElse(null);
	...
	public DocumentStore actualitzarDocument(
		...
		if (expedient.isArxiuActiu() && documentStore.getAnnexId() != null) {
			AnotacioAnnex annex = anotacioAnnexRepository.findById(documentStore.getAnnexId()).orElse(null);
...
//							jpd.getVersion());
					...
					if (doc != null) {
						...
						dto.setPortafirmesFluxNom(doc.getPortafirmesFluxNom());
						dto.setPortafirmesFluxTipus(doc.getPortafirmesFluxTipus());
					}
				}
				if (documentStore.getArxiuUuid() != null) {
					...
					if (ambContingutSignat && documentStore.isSignat() && isSignaturaFileAttached()) {
						...
						byte[] signatura = documentStore.getArxiuContingut(); //pluginHelper.custodiaObtenirSignaturesAmbArxiu(documentStore.getReferenciaCustodia());
						...
						if (documentStore.isSignat() && isSignaturaFileAttached()) {
							if (ambContingutSignat) {
								...
								arxiuOrigenNom = getNomArxiuAmbExtensio(
										documentStore.getArxiuNom(),
										getExtensioArxiuSignat());
								arxiuOrigenContingut = documentStore.getArxiuContingut(); //pluginHelper.custodiaObtenirSignaturesAmbArxiu(documentStore.getReferenciaCustodia());
							}
						} else {
...
//		}
	...
	private boolean isActiuConversioVista() {
		...
		if (!"true".equalsIgnoreCase(actiuConversio))
			return false;
		String actiuConversioVista = (String)GlobalProperties.getInstance().getProperty(PropertyConfig.PROP_CONVERSIO_VISTA_ACTIU);
		if (actiuConversioVista == null)
			actiuConversioVista = (String)GlobalProperties.getInstance().getProperty(PropertyConfig.PROP_CONVERSIO_GENTASCA_ACTIU);

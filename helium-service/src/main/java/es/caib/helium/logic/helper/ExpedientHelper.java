...
import es.caib.helium.persistence.entity.Expedient.IniciadorTipus;
...
//		List<JbpmProcessInstance> processInstancesTree = workflowEngineApi.getProcessInstanceTree(expedient.getProcessInstanceId());
		String[] ids = null;//		String[] ids = new String[processInstancesTree.size()];
//		int i = 0;
//		for (JbpmProcessInstance pi: processInstancesTree)
//			ids[i++] = pi.getId();
	...
	public Object comprovarInstanciaProces(
			Expedient expedient,
...
//								expedientPerRetornar.getId()));
	...
	public class RelacionarAnotacioAmbExpedientHandler implements TransactionSynchronization {
		...
		public void afterCommit() {
			...
			Anotacio anotacio = anotacioRepository.findById(anotacioId).orElse(null);
	...
	public Estat estatCanviar(Expedient expedient, Long estatId, boolean retrocedir) {
		...
		Estat estatFutur = estatRepository.findById(estatId).orElse(null);
			...
			if (AccioTipusEnumDto.HANDLER_PROPI.equals(accio.getTipus())) {
				try {
					HeliumActionHandler handler = recursHelper.createHandlerInstance(
						...
						accio.getHandlerClasse(),
						dades);
					handler.execute(heliumApiFactory.createInstance(expedient, null));

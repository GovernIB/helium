...
public class CampServiceImpl implements CampService {
	...
	public CampDto create(
		...
		if (expedientTipusId != null)
			entity.setExpedientTipus(expedientTipusRepository.findById(expedientTipusId).orElse(null));
		if (definicioProcesId != null)
			entity.setDefinicioProces(definicioProcesRepository.findById(definicioProcesId).orElse(null));

		// Dades consulta
		Enumeracio enumeracio = null;
		if (camp.getEnumeracio() != null) {
			enumeracio = enumeracioRepository.findById(camp.getEnumeracio().getId()).orElse(null);
		}
		entity.setEnumeracio(enumeracio);
		Domini domini = null;
		if (camp.getDomini() != null) {
			domini = dominiRepository.findById(camp.getDomini().getId()).orElse(null);
		}
		entity.setDomini(domini);
		Consulta consulta = null;
		if (camp.getConsulta() != null) {
			consulta = consultaRepository.findById(camp.getConsulta().getId()).orElse(null);
		}
	...
	public CampDto update(CampDto camp) throws NoTrobatException, PermisDenegatException {
		logger.debug(
				...
				"camp.id=" + camp.getId() + ", " +
				"camp =" + camp + ")");
		Camp entity = campRepository.findById(camp.getId()).orElse(null);
	...
	public void delete(Long campCampId) throws NoTrobatException, PermisDenegatException {
		logger.debug(
				...
				"campId=" + campCampId +  ")");
		Camp entity = campRepository.findById(campCampId).orElse(null);
	...
	public CampDto findAmbId(
		...
		CampDto dto = conversioTipusHelper.convertir(
				...
				CampDto.class);
		// Herencia
		ExpedientTipus tipus = expedientTipusId != null? expedientTipusRepository.findById(expedientTipusId).orElse(null) : null;
	...
	public CampAgrupacioDto agrupacioFindAmbId(Long id) throws NoTrobatException {
		logger.debug(
				...
				"campAgrupacioId=" + id +  ")");
		CampAgrupacio  agrupacio = campAgrupacioRepository.findById(id).orElse(null);
		if (agrupacio == null) {
	...
	public CampAgrupacioDto agrupacioUpdate(
		...
		logger.debug(
				...
				"agrupacio =" + agrupacio + ")");
		CampAgrupacio entity = campAgrupacioRepository.findById(agrupacio.getId()).orElse(null);
	...
	public void agrupacioDelete(Long agrupacioCampId) throws NoTrobatException, PermisDenegatException {
		...
		CampAgrupacio entity = campAgrupacioRepository.findById(agrupacioCampId).orElse(null);
		if (entity != null) {
	...
	private int reordenarAgrupacions(
		...
		List<CampAgrupacio> campsAgrupacio = expedientTipusId != null
				? expedientTipusRepository.findById(expedientTipusId).orElse(null).getAgrupacions()
				: definicioProcesRepository.findById(definicioProcesId).orElse(null).getAgrupacions();
		int i = 0;
	...
	public boolean afegirAgrupacio(
		...
		CampAgrupacio agrupacio = campAgrupacioRepository.findById(agrupacioId).orElse(null);
	...
	public CampRegistreDto registreCreate(
		...
		entity.setRegistre(campRepository.findById(campId).orElse(null));
		entity.setMembre(campRepository.findById(campRegistre.getMembreId()).orElse(null));
	...
	public CampRegistreDto registreUpdate(CampRegistreDto campRegistre)
		...
		CampRegistre entity = campRegistreRepository.findById(campRegistre.getId()).orElse(null);

		entity.setRegistre(campRepository.findById(campRegistre.getRegistreId()).orElse(null));
		entity.setMembre(campRepository.findById(campRegistre.getMembreId()).orElse(null));
		entity.setObligatori(campRegistre.isObligatori());
		entity.setLlistar(campRegistre.isLlistar());
	...
	public void registreDelete(Long campRegistreId) throws NoTrobatException, PermisDenegatException {
		...
		CampRegistre campRegistre = campRegistreRepository.findById(campRegistreId).orElse(null);
		campRegistre.getRegistre().getRegistreMembres().remove(campRegistre);
		campRegistreRepository.delete(campRegistre);
		campRegistreRepository.flush();
	...
	public boolean registreMourePosicio(
										...
										int posicio) {
		boolean ret = false;
		CampRegistre campRegistre = campRegistreRepository.findById(id).orElse(null);
	...
	public CampRegistreDto registreFindAmbId(Long id) throws NoTrobatException {
		logger.debug(
				...
				"campRegistreId=" + id +  ")");
		CampRegistre camp = campRegistreRepository.findById(id).orElse(null);
		if (camp == null) {

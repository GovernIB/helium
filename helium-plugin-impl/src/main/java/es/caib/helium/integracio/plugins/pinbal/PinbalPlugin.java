...
import es.caib.helium.commons.dto.ScspEstadoPinbal;
	...
	public Object peticioClientPinbalGeneric(DadesConsultaPinbal dadesConsultaPinbal) throws Exception {
		...
		try {
			if(dadesConsultaPinbal.isAsincrona()){
				...
				assert confirmacioPeticio != null;
...
//		solicitud.setAnioNacimiento(dadesConsultaPinbal.getAnyNaixement());
	...
	public ScspJustificantPinbal getJustificantPinbal(String idPeticio, String documentCodi) throws Exception {
	...
	private String getUsername() {
		return GlobalProperties.getInstance().getProperty(PropertyConfig.PROP_PINBAL_PLUGIN_USERNAME);
	}
	private String getPassword() {
		return GlobalProperties.getInstance().getProperty(PropertyConfig.PROP_PINBAL_PLUGIN_PASSWORD);
	}
	private boolean isJBoss() {
		if("false".equals(GlobalProperties.getInstance().getProperty(PropertyConfig.PROP_PINBAL_PLUGIN_ISJBOSS)))
			return false;
		else
			return true;
	}
	...
	private ScspConfirmacioPeticioPinbal convertirFromPinbalAsincrona(ScspConfirmacionPeticion scspRespuesta) throws Exception {
		...
		ScspAtributosPinbal atributos = new ScspAtributosPinbal();
		if (scspRespuesta.getAtributos()!=null) {
			...
			ScspEstadoPinbal estado = new ScspEstadoPinbal();

...
<c:choose>
	<c:when test="${not empty tasca.camps and not tasca.validada}">
		...
		<c:set var="tramitacioOnSubmit" value="alert('El formulari de la tasca encara no ha estat validat');return false"/>
	</c:when>
	<c:when test="${not empty tasca.documents and tasca.documentsNotReadOnly and not tasca.documentsComplet}">
		<c:set var="tramitacioClass" value="missatgesVermell"/>
		<c:set var="tramitacioOnSubmit" value="alert('Hi ha documents obligatoris sense adjuntar');return false"/>
	</c:when>
	<c:when test="${not empty tasca.signatures and not tasca.signaturesComplet}">
		<c:set var="tramitacioClass" value="missatgesVermell"/>
		<c:set var="tramitacioOnSubmit" value="alert('Hi ha documents obligatoris sense signar');return false"/>
	</c:when>
	<c:otherwise>

...
-- !!! INICI NOMÉS PER RECTIFICAR A DEV

-- Modificar la columna tipus per a que el tipus per defecte sigui ACCIO
ALTER TABLE HEL_ACCIO MODIFY TIPUS VARCHAR2(32 CHAR) DEFAULT 'ACCIO' NOT NULL;
-- Reaonomenar PREDEFINIT_CLASSE  a HANDLER_CLASSE
ALTER TABLE HEL_ACCIO RENAME COLUMN PREDEFINIT_CLASSE TO HANDLER_CLASSE;
-- Reaonomenar PREDEFINIT_DADES  a HANDLER_DADES
ALTER TABLE HEL_ACCIO RENAME COLUMN PREDEFINIT_DADES TO HANDLER_DADES;

-- Corregeix les accions existents
UPDATE HEL_ACCIO SET TIPUS = 'HANDLER_PROPI' ,
	HANDLER_CLASSE = JBPM_ACTION ,
	JBPM_ACTION = NULL 
WHERE TIPUS = 'HANDLER' 
	AND ID IN (
	SELECT a.id 
	FROM HEL_ACCIO a
		INNER JOIN HEL_EXPEDIENT_TIPUS et ON a.EXPEDIENT_TIPUS_ID = et.ID 
	WHERE 
		et.TIPUS  = 1
);
...
--> update de les accions a DES i DEV, els handlers passen a ser accions i els handlers proppis a acció 

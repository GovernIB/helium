...
public class SpringMassiuExecutor implements Serializable {
	...
	protected Thread createThread(String threadName) {
		...
		try { 
			waitTime = Integer.parseInt(GlobalProperties.getInstance().getProperty("app.massiu.periode.noves")); 
		} catch (Exception ex) {}
		try {
			timeBetweenExecutions = Integer.parseInt(GlobalProperties.getInstance().getProperty("app.massiu.periode.execucions")); 
		} catch (Exception ex) {}
	...
	public void setExecucioMassivaService(

...
public class MesuresTemporalsHelper {
	...
	protected class Clau implements Comparable<Clau> {
		...
		public int compareTo(Clau o) {
			int comp = compare(this.nom, o.nom);
			if (comp == 0) {
				comp = compare(this.tipusExpedient, o.tipusExpedient);
				if  (comp == 0) {
					comp = compare(this.tasca, o.tasca);
					return comp == 0 ? compare(this.detall, o.detall) : comp;
				}
			} 
			return comp;
		}

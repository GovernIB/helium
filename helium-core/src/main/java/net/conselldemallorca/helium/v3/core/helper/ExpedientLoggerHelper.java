					...
					if (logo.getName() != null) {
						...
						if (created && !deleted) {
							...
							jbpmHelper.deleteProcessInstanceVariable(
									pid,
									logo.getName());
							if (logo.getName().startsWith(DocumentHelperV3.PREFIX_VAR_DOCUMENT)) {
...
//			Node ndesti = jbpmHelper.getNodeByName(expedientLog.getProcessInstanceId(), desti);
		...
		if (retrocedirPerTasques && nodeEnterTokenId != null) {
			...
			for (DocumentTasca document: getDocumentsPerTaskInstance(ti)) {
				String codi = DocumentHelperV3.PREFIX_VAR_DOCUMENT + document.getDocument().getCodi();
				if (!document.isReadOnly()) {
					Object valor = ci.getVariable(DocumentHelperV3.PREFIX_VAR_DOCUMENT + document.getDocument().getCodi());
					if (valor != null)
						if (debugRetroces)

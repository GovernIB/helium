...
public class ConfiguracioParametresController extends BaseController {
	...
	public String post (
		...
		if (Accions.GUARDAR.equals(accio)) {
			...
			aplicacioService.getGlobalProperties().setProperty(APP_CONFIGURACIO_REDIRECCIONAR, String.valueOf(parametresCommand.isRedireccionar()));
			...
			for (String valorDefecte : valorsDefecte.keySet()) 
				aplicacioService.getGlobalProperties().setProperty(valorDefecte, valorsDefecte.get(valorDefecte));
	...
	private boolean isRedireccionar() {
		...
		if (!valorsDefecte.containsKey(APP_CONFIGURACIO_REDIRECCIONAR)) {
			valorsDefecte.put(APP_CONFIGURACIO_REDIRECCIONAR, String.valueOf("true".equalsIgnoreCase(aplicacioService.getGlobalProperties().getProperty(APP_CONFIGURACIO_REDIRECCIONAR))));
		}
		return "true".equalsIgnoreCase(aplicacioService.getGlobalProperties().getProperty(APP_CONFIGURACIO_REDIRECCIONAR));
	}

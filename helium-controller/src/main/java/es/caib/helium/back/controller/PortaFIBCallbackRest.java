...
public class PortaFIBCallbackRest {
	...
	public ResponseEntity<String> event(@RequestBody PortaFIBEvent event) {
		...
		default:
			...
			adminService.monitorAddAccio("INTCODI_PFIRMA", accioDescripcio,
		...
		if (portasignatures != null) {
			...
			try {
				...
				case SIGNAT:
					accio = "Signat";
					portasignaturesService.processarDocumentCallback(documentId.intValue(), false, null);
					processamentOk = true;
				...
				case REBUTJAT:
					...
					processamentOk = true;
					portasignaturesService.processarDocumentCallback(documentId.intValue(), true, motiu);
					resposta = (processamentOk) ? 1D : -1D;
					break;
				default:
					break;

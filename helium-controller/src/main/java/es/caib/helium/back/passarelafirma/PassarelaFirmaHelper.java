...
public class PassarelaFirmaHelper {
	...
	public String iniciarProcesDeFirma(
		...
		FileInfoSignature fis = getFileInfoSignature(
				...
				aplicacioService.getArxiuMediaType(fitxerPerFirmar.getNom()),
...
//	}
	...
	private List<PassarelaFirmaPlugin> getAllPluginsFromProperties() {
		if (plugins == null) {
			...
			String idsStr = aplicacioService.getGlobalProperties().getProperty(PROPERTIES_BASE + "ids");
			log.info("Identificadors de plugins: " + idsStr);
			if (idsStr != null && !idsStr.isEmpty()) {
				String[] ids = idsStr.split(",");
				for (String id: ids) {
					String base = PROPERTIES_BASE + id + ".";
					String nom = aplicacioService.getGlobalProperties().getProperty(base + "nom");
					log.info("Plugin a carregar NOM[" + base + "nom" + "]: " + nom);
					String classe = aplicacioService.getGlobalProperties().getProperty(base + "class");
					if (classe != null) {
						String descripcioCurta = aplicacioService.getGlobalProperties().getProperty(base + "desc");
						Properties pluginProperties = aplicacioService.getGlobalProperties().findByPrefix(base);
						...
						for (Object property: pluginProperties.keySet()) {
							...
							if (propertyStr.startsWith(base)) {
								String value = aplicacioService.getGlobalProperties().getProperty(propertyStr);

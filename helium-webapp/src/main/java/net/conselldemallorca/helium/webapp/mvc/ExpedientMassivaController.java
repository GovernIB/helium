...
//					getIdExecucionsMassivesActives(request);
	...
	@RequestMapping(value = "/expedient/reprendreMas")
	public String accioReprendreMas(
			HttpServletRequest request,
			@RequestParam(value = "inici", required = true) String inici,
			@RequestParam(value = "correu", required = true) String correu,
			ModelMap model) {
		...
		if (entorn != null) {
			...
			if (potModificarExpedient(expedientPrimer)) {
				try {
					...
					dto.setExpedientIds(ids.subList(1, ids.size()));
					dto.setExpedientTipusId(ids.get(0));
					dto.setTipus(ExecucioMassivaTipus.REPRENDRE);
					execucioMassivaService.crearExecucioMassiva(dto);
					
					missatgeInfo(request, getMessage("info.accio.massiu.reprendre", new Object[] {numExp}));
				} catch (Exception e) {
					missatgeError(request, getMessage("error.no.massiu"));
					logger.error("Error al programar les accions massives", e);
				}
	...
	@RequestMapping(value = "/expedient/reprendreExpedientMas")
	public String accioReprendreExpedientMas(
			HttpServletRequest request,
			@RequestParam(value = "inici", required = true) String inici,
			@RequestParam(value = "correu", required = true) String correu,
			ModelMap model) {
		...
		if (entorn != null) {
			...
			if (potModificarExpedient(expedientPrimer)) {
				try {
					...
					dto.setExpedientIds(ids.subList(1, ids.size()));
					dto.setExpedientTipusId(ids.get(0));
					dto.setTipus(ExecucioMassivaTipus.REPRENDRE_EXPEDIENT);
					execucioMassivaService.crearExecucioMassiva(dto);
					
					missatgeInfo(request, getMessage("info.accio.massiu.reprendre_expedient", new Object[] {numExp}));
				} catch (Exception e) {
					missatgeError(request, getMessage("error.no.massiu"));
					logger.error("Error al programar les accions massives", e);
				}

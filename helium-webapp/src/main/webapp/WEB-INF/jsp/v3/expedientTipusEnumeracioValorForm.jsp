...
<c:choose>
	<c:when test="${empty expedientTipusEnumeracioValorCommand.id}"><
		<c:set var="titol"><spring:message code="expedient.tipus.enumeracio.valors.form.titol.nou"/></c:set>
		<c:set var="formAction">new</c:set>
	</c:when>
	<c:otherwise>
		<c:set var="titol"><spring:message code="expedient.tipus.enumeracio.valors.form.titol.modificar"/></c:set>
...
<body>		
	...
	<form:form cssClass="form-horizontal" action="${formAction}" enctype="multipart/form-data" method="post" commandName="expedientTipusEnumeracioValorCommand">

		<div>
			<input type="hidden" name="id" value="${expedientTipusEnumeracioValorCommand.id}"/>

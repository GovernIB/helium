...
<table id="log_${log.id}" class="table tableLogs table-bordered">
	<thead>
		<tr>
			...
			<c:if test="${isAdmin || (tipus_retroces == 0 && not isAdmin)}">
			...
			<th>
				...
				<c:if test="${not (tipus_retroces == 0 && not isAdmin)}">
...
<c:choose>
	<c:when test="${((log.targetTasca && not isAdmin) || tipus_retroces == 0) || isAdmin}">
		<tr>
			<td>

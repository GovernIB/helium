				...
				<c:forEach var="registre" items="${expedient.relacionats}">
					...
					<td style="${filaStyle}">
						<security:accesscontrollist domainObject="${expedient.tipus}" hasPermission="16,8">
							<a href="<c:url value="${expedient.id}/relacioDelete"><c:param name="expedientIdOrigen" value="${expedient.id}"/><c:param name="expedientIdDesti" value="${registre.id}"/></c:url>" onclick="return confirmarEsborrarRelacio(event)"><img src="<c:url value="/img/cross.png"/>" alt="<spring:message code='comuns.esborrar' />" title="<spring:message code='comuns.esborrar' />" border="0"/></a>
...
function confirmarEsborrarRelacio(e) {
	...
	return confirm("<spring:message code='expedient.info.confirm.relacio.esborrar'/>");
}
// ]]>
</script>
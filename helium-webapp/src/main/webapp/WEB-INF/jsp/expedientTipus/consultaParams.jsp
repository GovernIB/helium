...
<body>
	<display:table name="llistat" id="consultaParam" requestURI="" class="displaytag selectable">
		...
		<display:column>
			<a href="<c:url value="/expedientTipus/consultaParamsDelete.html"><c:param name="id" value="${consulta.id}"/><c:param name="paramId" value="${consultaParam.id}"/><c:param name="expedientTipusId" value="${param.expedientTipusId}"/></c:url>" onclick="return confirmar(event)"><img src="<c:url value="/img/cross.png"/>" alt="<fmt:message key='comuns.esborrar' />" title="<fmt:message key='comuns.esborrar' />" border="0"/></a>
		</display:column>
	</display:table>
	<form:form action="consultaParams.html" method="post" cssClass="uniForm">

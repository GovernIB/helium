...
<c:choose>
	<c:when test="${empty tokenActual}"><a href="<c:url value="/document/arxiuMostrar.html"><c:param name="id" value="${documentActual.id}"/></c:url>"><img src="<c:url value="/img/page_white_put.png"/>" alt="<fmt:message key='comuns.descarregar' />" title="<fmt:message key='comuns.descarregar' />" border="0"/></a></c:when>
	<c:otherwise><a href="<c:url value="/document/arxiuPerSignar.html"><c:param name="token" value="${tokenActual}"/><c:param name="noe" value="true"/></c:url>"><img src="<c:url value="/img/page_white_put.png"/>" alt="<fmt:message key='comuns.descarregar' />" title="<fmt:message key='comuns.descarregar' />" border="0"/></a></c:otherwise>
</c:choose>
<c:if test="${documentActual.signat and empty ocultarSignatura}">
	<a href="<c:url value="/signatura/verificarIntern.html"><c:param name="token" value="${documentActual.tokenSignatura}"/></c:url>" onclick="return verificarSignatura(this)"><img src="<c:url value="/img/rosette.png"/>" alt="<fmt:message key='common.icon.verif_signa' />" title="<fmt:message key='common.icon.verif_signa' />" border="0"/></a>
</c:if>

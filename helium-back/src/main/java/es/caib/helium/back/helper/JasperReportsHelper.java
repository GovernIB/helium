...
public class JasperReportsHelper {
	 ...
	 */
	public InformeHelper.InformeInfo generarReport(
		...
		if (datasource != null) {
			...
			if (subreports!=null && !subreports.isEmpty()) {
				Iterator<Map.Entry<String, byte[]>> it = subreports.entrySet().iterator();
				while (it.hasNext()) {
			...
			info.setEstat(InformeHelper.Estat.GENERANT);
	...
	public void esperarGeneracio(InformeHelper.InformeInfo info) {
	...
	public void notificarFinalitzat(InformeHelper.InformeInfo info) {
		JasperReportInfo jasperInfo = this.jasperInfo.get(info.getId());
		if (jasperInfo != null) {
			synchronized(jasperInfo.o) {
				try {
	 ...
	 * @throws JRException */
	public InformeHelper.InformeInfo cancellar(InformeHelper.InformeInfo info) throws JRException {
	...
	public class JasperReportInfo {
		...
		private InformeHelper.InformeInfo info;
		...
		public InformeHelper.InformeInfo getInformeInfo() {
			return info;
		}
		// Per mètriques
	...
	public void setJasperPrint(InformeHelper.InformeInfo informeInfo, JasperPrint jasperPrint) {

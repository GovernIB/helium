...
public class ExpedientConsultaInformeController extends BaseExpedientController {
	...
	@ResponseBody
	public InformeHelper.InformeInfo informeParamsAsync(
		HttpServletRequest request,
		HttpServletResponse response,
		@PathVariable Long consultaId,
		...
		InformeHelper.InformeInfo ret = informeHelper.getConsulta(session.getId());

		if (InformeHelper.Estat.NO_TROBAT.equals(ret.getEstat())) {
			ret.setId(request.getSession().getId());
			ret.setEstat(InformeHelper.Estat.INICIALITZANT);
			...
			Map<String, Object> valors = TascaFormHelper.getValorsFromCommand(
				...
				parametrosCommand,
				true);
			InformeHelper.InformeInfo info = new InformeHelper().new InformeInfo();
			info.setEstat(InformeHelper.Estat.INICIALITZANT);
	...
	@ResponseBody
	public InformeHelper.InformeInfo informeAsync(
		HttpServletRequest request,
		@PathVariable Long consultaId,
		HttpSession session,
		Model model) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, IOException {
	...
	@ResponseBody
	public InformeHelper.InformeInfo informeAsyncInfo(
		HttpServletRequest request,
		@PathVariable Long consultaId,
		HttpSession session,
		Model model) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, IOException {

		InformeHelper.InformeInfo info = informeHelper.getConsulta(request.getSession().getId());
	...
	@ResponseBody
	public InformeHelper.InformeInfo informeAsyncCancellar(
		HttpServletRequest request,
		@PathVariable Long consultaId,
		HttpSession session,
		Model model) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, IOException {
		...
		InformeHelper.InformeInfo info = informeHelper.getConsulta(request.getSession().getId());
		try {
			if (InformeHelper.Estat.GENERANT.equals(info.getEstat())) {
				info = jasperReportsHelper.cancellar(info);
			}
			info.setEstat(InformeHelper.Estat.CANCELLAT);
			informeHelper.setConsulta(info.getId(), null);
			jasperReportsHelper.setJasperPrint(info, null);
		} catch (JRException e) {
			String msg = getMessage(request, "error.consulta.informe.cancelacio", new Object[] {e.getMessage()});
			logger.error(msg, e);
			info.setEstat(InformeHelper.Estat.ERROR);
	 ...
	 * @throws IOException
	 */
	private InformeHelper.InformeInfo generarReport(
		...
		for (int i = 0; i < ExpedientConsultaInformeController.MAX_ORDER_COLUMNS; i++) {
			if(i >= campsConsulta.size() || campsConsulta.isEmpty())
				break;
			paginacio.afegirOrdre(campsConsulta.get(i).getVarCodi(), PaginacioParamsDto.OrdreDireccioDto.ASCENDENT);
		}

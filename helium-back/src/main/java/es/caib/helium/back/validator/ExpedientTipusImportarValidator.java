...
import es.caib.helium.commons.exportacio.CampTascaExportacio;
...
import es.caib.helium.commons.exportacio.DocumentTascaExportacio;
import es.caib.helium.commons.exportacio.ExpedientTipusExportacio;
import es.caib.helium.commons.exportacio.FirmaTascaExportacio;
...
//	    					accionsDefinicio.put(definicio.getDefinicioProcesDto().getJbpmKey(), processDefinition.getActions().keySet());
			...
			if (command.getId() == null) {
				...
				if (command.getCodi() == null || "".equals(command.getCodi())) {
					...
					// Comprova que no existeixi ja un tipus d'expedient amb el mateix codi en tots els entorns
					...
					if(repetits!=null && !repetits.isEmpty()) {
						...
						for(ExpedientTipusDto repetit: repetits) {
							if(repetit != null && (command.getId() == null || !command.getId().equals(repetit.getId()))) {
								...
								context.buildConstraintViolationWithTemplate(
										MessageHelper.getInstance().getMessage("expedient.tipus.importar.validacio.codi.repetit.entorn",

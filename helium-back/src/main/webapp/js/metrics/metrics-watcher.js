...
(function(metricsWatcher, $, undefined) {
	...
	*/
	...
	metricsWatcher.clearGraphs = function() {
		graphs = [];
	};
	...
	function drawCounter(counterInfo) {
		...
		var html = 	"<div class='metricsWatcher counter counterGraph'>" + 
					"	<div class='counterTitle'>" + counterInfo.title + "</div>" +
					"	<div class='progress'>" +
...
//			progressClass = "progress-bar-success";
		...
		$("#" + counterInfo.divId + " div.progress div.progress-bar").css("width", pct + "%").addClass(progressClass);
		$("#" + counterInfo.divId + " div.progress div.progress-bar").html(metricData.count);
//		$("#" + counterInfo.divId + " div.progress span.progress-title").html(metricData.count);// + "/" + counterInfo.getMax(json));
	}
 ...
 */
	...
	function updateJvm(jvmInfo, json) {
		...
		var html = "<div class='row'>"
				...
				+ (!jvm['jvm.gc.MarkSweepCompact.count']?"":("<tr><td><h5>PS Mark Sweep Runs</h5></td><td>" + jvm['jvm.gc.MarkSweepCompact.count'].value + "</td></tr>"))
				+ (!jvm['jvm.gc.MarkSweepCompact.time']?"":("<tr><td><h5>PS Mark Sweep Time</h5></td><td>" + jvm['jvm.gc.MarkSweepCompact.time'].value + "</td></tr>"))
				+ (!jvm['jvm.gc.Copy.count']?"":("<tr><td><h5>GC Copy Runs</h5></td><td>" + jvm['jvm.gc.Copy.count'].value + "</td></tr>"))
				+ (!jvm['jvm.gc.Copy.time']?"":("<tr><td><h5>GC Copy Time</h5></td><td>" + jvm['jvm.gc.Copy.time'].value + "</td></tr>"))
				+ "</table></div>"

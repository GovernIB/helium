				...
				<tr <c:if test="${empty document.id}">class="hide"</c:if>>
					...
					<td class="tableDocumentsTd">
						<c:if test="${document.documentStoreId != null }">
							<a href="<c:url value="/expedient/${tasca.expedientId}/proces/${tasca.processInstanceId}/document/${document.documentStoreId}/descarregar"/>"><span class="fa fa-download" title="<spring:message code="comuns.descarregar"/>"></span></a>
							...
							<c:if test="${document.registrat}">
								<a 	data-rdt-link-modal="true" 
									class="icon registre" 
									href="<c:url value='/modal/expedient/${expedientId}/verificarRegistre/${document.documentStoreId}/${document.documentCodi}'/>">

						...
						<c:when test="${not empty document.tokenSignatura}">
							<c:choose>
								<c:when test="${not document.signat}">
									<c:url value="/tasca/${tasca.id}/document/${document.documentCodi}/descarregar" var="downloadUrl"></c:url>
								...
								<div id="firmar${document.id}">
									<c:if test="${not document.signat}">
										...
										<div id="botons${document.id}" class="modal-botons-firma">
											<button type="button" onclick="obrirFinestraFirma('<c:url value="/modal/tasca/${tasca.id}/document/${document.id}/firmaPassarela"/>');" class="btn btn-default"><spring:message code="tasca.signa.signar.passarela"/></button>
										</div>
									</c:if>
								</div>
							</c:if>
...
function refreshSignatures() {
	window.location.href = '<c:url value="/modal/tasca/${tasca.id}/signatura"/>';
}

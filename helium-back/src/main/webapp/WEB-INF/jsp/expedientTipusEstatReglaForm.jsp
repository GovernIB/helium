...
<head>
	...
	<script>
		...
		const configQuiValor = (qui) => {
			if (qui !== 'USUARI') {
				...
				$("#quiValor").select2({
					...
					ajax: {
						url: (params) => { return "<c:url value="/expedientTipus/persona/suggest/"/>" + params.term; },
						dataType: 'json',
						processResults: (data) => {
							let results = [];
		...
		const ompleQueVariables = (que) => {
			...
			let getUrl = '';
			if (que === 'DADA') {
				getUrl = '<c:url value="/expedientTipus/${estatReglaCommand.expedientTipusId}/var/select"/>';
			} else if (que === 'DOCUMENT') {
				getUrl = '<c:url value="/expedientTipus/${estatReglaCommand.expedientTipusId}/doc/select"/>';
			} else if (que === 'TERMINI') {
				getUrl = '<c:url value="/expedientTipus/${estatReglaCommand.expedientTipusId}/term/select"/>';
			} else if (que === 'AGRUPACIO') {
				getUrl = '<c:url value="/expedientTipus/${estatReglaCommand.expedientTipusId}/agrup/select"/>';
			} else {
				return;
			}

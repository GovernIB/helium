	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('#var').on('change', function() {
				...
				$("a[name='modificar_variable']").attr("href","../../expedient/massiva/"+$("#var").val()+"/modificarVariables?inici="+$("#inici").val()+"&correu="+$("#correu").is(":checked"))
			});
			$('#docId').on('change', function() {
				...
				if (docId == "") {
			 		...
			 		$("a[name='document_modificar']").attr("href","../../expedient/massiva/documentMassiu?inici="+$("#inici").val()+"&correu="+$("#correu").is(":checked")+"&docId="+docId+"&adjuntar=false");
			 		$("button[value='document_esborrar']").prop('disabled', false);
			 	}
				$("a[name='document_adjuntar_massiu']").attr("href","../../expedient/massiva/documentMassiu?inici="+$("#inici").val()+"&correu="+$("#correu").is(":checked")+"&docId="+docId+"&adjuntar=true");
			...
			$('#grup-default-titol').click( function() {
				...
				panell.load('<c:url value="/nodeco/expedient/massiva/expedientsSeleccio"/>');
			});
		});
		// ]]>
	</script>
...
<body>
	...
	<div class="opcionMasiva control-group right">
		<div class="label-titol">
			...
			<div class="form-group">
				<form:form cssClass="form-horizontal form-tasca" id="anularExpedientMas" name="anularExpedientMas" action="massiva/anularExpedientMas" method="post" modelAttribute="expedientEinesAnularCommand" onsubmit="return confirmarAnular(event)">
			...
			<div class="form-group">
				<c:if test="${not empty documents}">
					<form:form cssClass="form-horizontal form-tasca" id="documentMas" name="documentMas" action="massiva/documentMas" method="post" modelAttribute="documentExpedientCommand" onsubmit="return confirmarModificarDocument(event)">
						...
						<a data-rdt-link-modal-min-height="500" class="btn btn-primary right" name="document_adjuntar_massiu" href="../../expedient/massiva/documentMassiu" data-rdt-link-modal="true"><spring:message code='expedient.document.adjuntar_document_massiu' /></a>
					</form:form>
				</c:if>

...
$(document).ready(function() {
	var taulaDadesExpedient = $("#taulaDades").heliumDataTable({
		ajaxSourceUrl: "<c:url value="/expedient/datatable"/>",
	...
	$('#expedientTipusId').on('change', function() {
		...
		$('#consultaTipo').hide();
		if ($(this).val()) {
			$.get('<c:url value="/expedient/estatsPerTipus/"/>' + $(this).val())
			.done(function(data) {
				$('#estatText').append('<option value="<%=es.caib.helium.commons.dto.ExpedientDto.EstatTipusDto.INICIAT%>"><spring:message code="comu.estat.iniciat"/></option>');
				for (var i = 0; i < data.length; i++) {
					$('#estatText').append('<option value="' + data[i].id + '">' + data[i].nom + '</option>');
				}
				$('#estatText').append('<option value="<%=es.caib.helium.commons.dto.ExpedientDto.EstatTipusDto.FINALITZAT%>"><spring:message code="comu.estat.finalitzat"/></option>');
	...
	<c:set var="refrescaSegonPla" value="${propSegonplaRefrescarAuto == 'false' ? false : true}"/>
	<c:set var="refrescaSegonPlaPeriode" value="${propSegonplaRefrescarAutoPeriode != null ? propSegonplaRefrescarAutoPeriode : 10}"/>
...
function refrescarPanell(expedientId, tascaId, tramitar, correcte) {
	if (correcte) {
		...
		var jqxhr = $.ajax({
			url: "<c:url value="/nodeco/expedient/"/>" + expedientId + "/tasquesPendents/"+$('#nomesTasquesPersonals').val()+"/"+$('#nomesTasquesGrup').val(),
...
function carregaTasques(row, numTds) {
	var jqxhr = $.ajax({
		url: "<c:url value="/nodeco/expedient/"/>" + $(row).find(".rdt-seleccio").val() + "/tasquesPendents/"+$('#nomesTasquesPersonals').val()+"/"+$('#nomesTasquesGrup').val(),
		beforeSend: function(xhr) {
		...
		<thead>
			<tr>
				...
				<th data-rdt-property="estat.nom" data-rdt-template="cellEstatTemplate" data-rdt-visible="true">
					...
					<div class="pull-right">
						{{if ambErrors}}
							<a class="no-deco-link" data-rdt-link-modal="true" data-rdt-link-modal-maximize="true" href="<c:url value="../expedient/{{:id}}/errors"/>">
					...
					<script id="cellAccionsTemplate" type="text/x-jsrender">
						<div class="dropdown navbar-right">
							...
							<ul class="dropdown-menu">
								...
								{{if !aturat && (permisStop)}}<li><a href="../expedient/{{:id}}/aturar" data-rdt-link-modal="true"><span class="fa fa-pause"></span>&nbsp;<spring:message code="expedient.llistat.accio.aturar"/></a></li>{{/if}}
								{{if aturat && (permisStop)}}<li><a href="../expedient/{{:id}}/reprendre" data-rdt-link-callback="recarregarTaula(taulaDades);" data-rdt-link-ajax="true" data-rdt-link-confirm="<spring:message code="expedient.eines.confirm_reprendre_tramitacio"/>"><span class="fa fa-play"></span>&nbsp;<spring:message code="expedient.llistat.accio.reprendre"/></a></li>{{/if}}
								{{if !anulat && (permisCancel)}}<li><a href="../expedient/{{:id}}/anular" data-rdt-link-modal="true"><span class="fa fa-times"></span>&nbsp;<spring:message code="expedient.llistat.accio.anular"/></a></li>{{/if}}
								{{if anulat && (permisCancel)}}<li><a href="../expedient/{{:id}}/activar" data-rdt-link-callback="recarregarTaula(taulaDades);" data-rdt-link-ajax="true" data-rdt-link-confirm="<spring:message code="expedient.consulta.confirm.desanular"/>"><span class="fa fa-check"></span>&nbsp;<spring:message code="expedient.llistat.accio.activar"/></a></li>{{/if}}
								{{if permisDelete}}<li><a href="../expedient/{{:id}}/delete" data-rdt-link-ajax="false" data-rdt-link-confirm="<spring:message code="expedient.llistat.confirmacio.esborrar"/>"><span class="fa fa-trash-o"></span>&nbsp;<spring:message code="expedient.llistat.accio.esborrar"/></a></li>{{/if}}
							</ul>
						</div>
					</script>
				</th>

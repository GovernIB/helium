		...
		rowClickCallback: function(row, event) {
			...
			if (!isTdDesplegar && !clickNomesDesplegar) {
				...
				if ($(row).next().hasClass('table-pendents') || $(row).next().hasClass('tasques-pendents')) {
					...
					var jqxhr = $.ajax({
						url: "<c:url value="/nodeco/expedient/"/>" + $(row).find(".rdt-seleccio").val() + "/tasquesPendents/"+$('#nomesTasquesPersonals').val()+"/"+$('#nomesTasquesGrup').val()+"/consultesTipus",
...
<body>
	...
	<script type="text/javascript">
		...
		function getConsultaInfo() {
			...
			$.ajax({
				type: 'GET',
				url: '<c:url value="/expedient/consulta/${consulta.id}/informeAsync/info"/>',
				dataType: "json",
				traditional: true,
				async: false,
			  	data: {
		...
		function actualitzarInfoDescarrega(info) {
			...
			if (info == null || info.estat == 'NO_TROBAT') {
				...
				case 'FINALITZAT':
					...
					window.location = '<c:url value="/expedient/consulta/${consulta.id}/informeAsync/exportar"/>';
					informe = null;
					$('#informeDescarregarModal').modal('hide');
					break;
				case 'CANCELLAT':
		...
		function cancellarInforme() {
			...
			$.ajax({
				type: 'GET',
				url: '<c:url value="/expedient/consulta/${consulta.id}/informeAsync/cancellar"/>',
				dataType: "json",
				traditional: true,
				async: false,
			  	data: {

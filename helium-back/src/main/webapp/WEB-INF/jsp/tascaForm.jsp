	...
	<c:when test="${not empty tasca.tascaFormExternCodi}">
		<div class="alert alert-warning">
			<p>
				...
				<a id="boto-formext" href="<c:url value="/tasca/${tasca.id}/formExtern"/>" class="btn btn-xs btn-default pull-right"><span class="fa fa-external-link"></span> <spring:message code="tasca.form.obrir_form"/></a>
			</p>
		</div>
	</c:when>
...
<c:choose>
	...
	<c:otherwise><c:url var="tascaFormAction" value="/modal/tasca/${tasca.id}"/></c:otherwise>
</c:choose>
 -->
		  ...
		  <ul class="dropdown-menu dropdown-menu-right">
			...
			<c:if test="${not empty repros}">
			    ...
			    <c:forEach var="repro" items="${repros}">
				    <li class="flex">
				    	<a id="repro-${repro.id}" href="<c:url value="/modal/tasca/"/>${tasca.id}?reproId=${repro.id}">${repro.nom}</a>
...
$('.dropdown-menu').find('.reproForm').click(function (e) {
				...
				$('#guardarRepro').click(function(e) {
					...
					if ($('#nomRepro').val() != '') {
						$('#command').attr('action','<c:url value="/repro/"/>${tasca.expedientTipusId}/${tasca.definicioProcesId}/saveRepro');
						return true;
					} else {
						return false;
					}
				...
				$('.borrarRepro').click(function(e) {
					...
					$('#command').attr('action','<c:url value="/repro/"/>${tasca.expedientTipusId}/${tasca.definicioProcesId}/deleteRepro/' + reproId);
					return true;
				});

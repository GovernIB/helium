...
<body>
	...
	<c:if test="${errorMetadadesNti}">
		<div class="row alert alert-danger" style="margin: 0px; margin-bottom: 10px;">
			...
			<div class="col-sm-2">
				<form  action="<c:url value="/expedient/${expedient.id}/metadadesNti/arreglar"/>" 
					method="post">
								...
								<tr>
									...
									<td>
										...
										<c:forEach var="versio" items="${arxiuDetall.versionsDocument}" varStatus="status">
											<a href="<c:url value="/expedient/${expedientId}/proces/${expedientDocument.processInstanceId}/document/${expedientDocument.id}/descarregar/versio/${versio.eniVersio}/${arxiuDetall.expedientTancat}"/>">		
									...
									<td>
										<c:choose>
											...
											<c:when test="${firma.tipus == 'XADES_DET' or firma.tipus == 'CADES_DET'}">
												<a href="<c:url value='/expedient/${expedientId}/proces/${expedientDocument.processInstanceId}/document/${expedientDocument.id}/firma/${status.index}/descarregar'></c:url>" class="btn btn-default btn-sm pull-right">
	...
	<div id="modal-botons" class="well">
		<button type="button" class="btn btn-default modal-tancar" name="submit" value="cancel" data-modal-cancel="true"><spring:message code="comu.boto.tancar"/></button>
	</div>
</body>
</html>

...
<body>
	<c:if test="${not empty tasca.tascaFormExternCodi}">
		<div class="alert alert-warning">
			<p>
				...
				<a id="boto-formext" href="<c:url value="/expedient/tasca/${expedientTipus.id}/${definicioProces.id}/${tasca.id}/formExtern"/>" class="btn btn-xs btn-default pull-right"><span class="fa fa-external-link"></span>&nbsp;<spring:message code='tasca.form.obrir_form' /></a>
			</p>
		</div>
	...
	<c:url var="post_url" value="/modal/expedient" />
		  ...
		  <ul class="dropdown-menu dropdown-menu-right">
			...
			<c:if test="${not empty repros}">
			    ...
			    <c:forEach var="repro" items="${repros}">
				    <li class="flex">
				    	<a id="repro-${repro.id}" href="<c:url value="/modal/expedient/iniciarForm/"/>${expedientTipus.id}/${definicioProces.id}?reproId=${repro.id}">${repro.nom}</a>
		...
		<div id="modal-botons">
			...
			// <![CDATA[
				...
				$('#guardarRepro').click(function(e) {
					...
					if ($('#nomRepro').val() != '') {
						$('#command').attr('action','<c:url value="/modal/repro/"/>${expedientTipus.id}/${definicioProces.id}/guardarRepro');
						return true;
					} else {
						return false;
					}
				...
				$('.borrarRepro').click(function(e) {
					...
					$('#command').attr('action','<c:url value="/modal/repro/"/>${expedientTipus.id}/${definicioProces.id}/borrarRepro/' + reproId);
					return true;
				});

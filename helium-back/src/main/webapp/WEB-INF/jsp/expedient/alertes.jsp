	...
	<table class="table table-striped table-bordered" data-rdt-seleccionable="true" data-rdt-seleccionable-columna="0">
		...
		<c:forEach var="alerta" items="${alertes}">
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${empty alerta.dataLectura}"><a title="<spring:message code="alerta.llistat.marcar_com_llegida"/>" href="../../../alerta/${alerta.id}/llegir"><span class="fa fa-eye fa-lg"></span></a></c:when>
						<c:otherwise><a title="<spring:message code="alerta.llistat.marcar_no_llegida"/>" href="../../../alerta/${alerta.id}/noLlegir"><span class="fa fa-eye-slash fa-lg text-danger"></span></a></c:otherwise>
					</c:choose>
				</td>
				<td><a title="<spring:message code="comuns.esborrar"/>" href="../../../alerta/${alerta.id}/esborrar" onclick="return confirmar(event)"><span class="fa fa-trash fa-lg"></span></a></td>
			</tr>
		</c:forEach>	
		</tbody>			       		
	</table>

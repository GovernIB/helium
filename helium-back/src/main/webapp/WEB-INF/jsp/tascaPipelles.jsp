...
<head>
	...
	<c:choose>
		<c:when test="${isModal}"><c:url var="tascaFormAction" value="/modal/tasca/${tasca.id}"/></c:when>
		<c:otherwise><c:url var="tascaFormAction" value="/tasca/${tasca.id}"/></c:otherwise>
	</c:choose>
<script>
$(document).ready(function() {
		...
		$(targetHref).load(
			...
			function (responseText, textStatus, jqXHR) {
				if (textStatus == 'error') {
					...
					$(".modal-title", parent.document).html('<a href="<c:url value="/expedient/${tasca.expedientId}"/>">${tasca.expedientIdentificadorEscaped}</a>: ${tasca.titolEscaped}');
				}
			}
		);
	});
	$(".tasca-boto").click(function(e) {
		...
		if (accio.indexOf('completar') == 0) {
			$.ajax({
		        type: 'POST',
		        url: "<c:url value='/tasca/${tasca.id}/isPermetreFinalitzar'/>",
		        cache: false,
		        contentType: false,
		        processData: false,
		        async: false,
	...
	<c:choose>
		<c:when test="${hasFormulari or hasDocuments or hasSignatures}">
			...
			<div class="tab-content">
				<c:if test="${hasFormulari}">
					<div id="tasca-form" class="tab-pane active" data-href="<c:url value="/nodeco/tasca/${tasca.id}/form?reproId=${reproId}"/>"></div>
				</c:if>
				<c:if test="${hasDocuments}">
					<c:choose>
						<c:when test="${isModal}"><div id="tasca-document" class="tab-pane" data-href="<c:url value="/nodeco/modal/tasca/${tasca.id}/document"/>"></div></c:when>
						<c:otherwise><div id="tasca-document" class="tab-pane" data-href="<c:url value="/nodeco/tasca/${tasca.id}/document"/>"></div></c:otherwise>
					</c:choose>
				</c:if>
				<c:if test="${hasSignatures}">
					<div id="tasca-signatura" class="tab-pane" data-href="<c:url value="/nodeco/tasca/${tasca.id}/signatura"/>"></div>
				</c:if>
			</div>
		</c:when>
		<c:otherwise>

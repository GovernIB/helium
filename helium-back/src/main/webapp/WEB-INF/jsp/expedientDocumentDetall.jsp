...
<script type="text/javascript">
	...
	<div class="tab-content">
		...
		<div id="contingut-documentDetall-signatura-${detall.documentStoreId}" class="tab-pane">
			...
			<dl class="dl-horizontal">
				...
				<c:if test="${not empty signatura.tokenSignatura}">
					<c:url value="/expedient/document/arxiuMostrar" var="downloadUrl"><c:param name="token" value="${signatura.tokenSignatura}"/></c:url>
						<dt>Verificació de la signatura</dt>
					...
					<c:forEach var="notificacio" items="${notificacions}" varStatus="status">
						<c:forEach var="enviament" items="${notificacio.enviaments}" varStatus="statuse">
							<tr>					
								...
								<td>
									<a href="<c:url value="/expedient/${expedient.id}/proces/${expedient.processInstanceId}/document/${notificacio.documentId}/descarregar"/>"
									...
									<br/><span class="">Conté:</span>
										...
										<c:forEach items="${notificacio.documentsDinsZip}" var="docContingut">
											<li data-id="${docContingut.id}" data-codi="${docContingut.codiDocument}">
												<a href="<c:url value="/expedient/${expedient.id}/proces/${expedient.processInstanceId}/document/${docContingut.id}/descarregar"/>"
								...
								<td>
									<c:if test="${not empty notificacio.justificantId}">
										<center>
										<a href="<c:url value="/expedient/${expedient.id}/proces/${expedient.processInstanceId}/document/${notificacio.justificantId}/descarregar"/>">
		...
		<%--					TODO:--%>
		<%--									<div class="col-sm-2">--%>
		<%--										<form  action="<c:url value="/expedient/${expedient.id}/metadadesNti/arreglar"/>" method="post">--%>
											...
											<td>
												<c:choose>
													...
													<c:when test="${firma.tipus == 'XADES_DET' or firma.tipus == 'CADES_DET'}">
														<a href="<c:url value='/expedient/${expedientId}/document/${detall.documentStoreId}/firma/${status.index}/descarregar'></c:url>" class="btn btn-default btn-sm pull-right">
											...
											$(document).ready(function() {
											    $("#collapse-registre-firmes-annex-${status.index}-anotacio-${anotacio.id}-document-${detall.documentStoreId}").on('show.bs.collapse', function(data){  	
												    if (!$(this).data("loaded")) {
												        ...
												        $(this).load('<c:url value="/nodeco/anotacio/${anotacio.id}/annex/"/>' + ${annex.id} + '/firmaInfo');
												...
												<div>
													<c:if test="${annex.estat == 'PENDENT' && not empty annex.error}">
														...
														<div class="alert well-sm alert-danger alert-dismissable" style="margin-bottom: 0px;">
															...
															<a href="<c:url value="/anotacio/${anotacio.id}/annex/${annex.id}/reintentar"/>"
	...
	<%--											<dl>--%>
	<%--												<a href="<c:url value="/anotacio/${anotacio.id}/reintentarNotificar"/>"--%>
	<%--												   class="btn btn-xs btn-default pull-right"><span class="fa fa-refresh"></span>--%>

				...
				<c:forEach var="i" begin="1" end="${mida}">
					<tr class="multiple">
						<c:forEach var="membre" items="${registreCap}" varStatus="status">
							<c:set var="campErrorLinia" value='<%= TascaFormValidatorHelper.getErrorField((org.springframework.validation.Errors)session.getAttribute("tascaError"), (es.caib.helium.commons.dto.TascaDadaDto)pageContext.getAttribute("dadaActual"), (javax.servlet.jsp.jstl.core.LoopTagStatus)pageContext.getAttribute("status")) %>'/>

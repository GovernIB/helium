...
<body>			
	...
	$(document).ready(function() {
		...
		$('#btnAddAccioEntrada').click(function() {
			...
			$.ajax({
			    url:"<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/entrada/add/"></c:url>" + accioId,
			    type:'POST',
			    dataType: 'json',
			    cache: false,
		...
		$('#btnAddAccioSortida').click(function() {
			...
			$.ajax({
			    url:"<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/sortida/add/"></c:url>" + accioId,
			    type:'POST',
			    dataType: 'json',
			    cache: false,
			    success: function(data) {
		...
		function canviarPosicioValidacio(taula, taulaId, id, pos) {
			...
			var getUrl = '<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/"></c:url>' + taula + '/' + id + '/moure/' + pos;
			$.ajax({
				type: 'GET',
				url: getUrl,
				async: true,
	...
	<fieldset>
		...
		<table	id="estatAccionsEntradaDataTable"
				...
				data-url="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/entrada/datatable"></c:url>"
			...
			<thead>
				<tr>
					...
					<th data-col-name="id" data-template="#cellEstatAccionsEntradaTemplate" data-orderable="false" width="10%">
						...
						<c:if test="${!heretat}">
					    	<a href="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/entrada/"></c:url>{{:id}}/delete"  class="btn btn-danger" data-toggle="ajax" data-confirm="<spring:message code="expedient.tipus.estat.accions.llistat.accio.esborrar.confirmacio"/>"><span class="fa fa-trash-o"></span></a>
						</c:if>
					</script>
					</th>
				</tr>
	...
	<fieldset>
		...
		<table	id="estatAccionsSortidaDataTable"
				data-toggle="datatable"
				data-url="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/sortida/datatable"></c:url>"
				data-paging-enabled="false"
				data-ordering="true"
				data-default-order="1"
				class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th data-col-name="id" data-template="#cellEstatAccionsSortidaTemplate" data-orderable="false" width="10%">
						...
						<c:if test="${!heretat}">
					    	<a href="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/accions/sortida/"></c:url>{{:id}}/delete"  class="btn btn-danger" data-toggle="ajax" data-confirm="<spring:message code="expedient.tipus.estat.accions.llistat.accio.esborrar.confirmacio"/>"><span class="fa fa-trash-o"></span></a>
						</c:if>
					</script>
					</th>
				</tr>

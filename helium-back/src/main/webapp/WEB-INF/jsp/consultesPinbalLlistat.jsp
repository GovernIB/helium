...
<body>
	...
	<form:form action="" method="post" cssClass="well" modelAttribute="consultesPinbalFiltreCommand">
...
(Entorn {{:entorn.codi}} - {{:entorn.nom}})"></span>
				...
				<th data-col-name="document.arxiuNom" data-template="#cellDocTemplate" width="13%"><spring:message code="consultes.pinbal.camp.document"/>
					<script id="cellDocTemplate" type="text/x-jsrender">
						{{if document!=null}}<a href="<c:url value="/expedient/{{:expedientId}}/document/{{:documentId}}/descarregar"/>" target="_blank">{{:document.arxiuNom}}</a>{{/if}}
					</script>
				</th>
					...
					<script id="cellAccionsTemplate" type="text/x-jsrender">
						<div class="dropdown navbar-right">
							...
							<ul class="dropdown-menu">
								<li><a href="<c:url value="/consultesPinbal/{{:id}}/info"/>" data-toggle="modal" class="consultar-expedient"><span class="fa fa-info-circle"></span>&nbsp;<spring:message code="consultes.pinbal.boto.info"/></a></li>
								{{if estat=='PENDENT' || estat=='ERROR_PROCESSANT'}}<li><a href="<c:url value="/consultesPinbal/{{:id}}/actualitzarEstat"/>" data-toggle="ajax"><span class="fa fa-refresh"></span>&nbsp;<spring:message code="consultes.pinbal.boto.update"/></a></li>{{/if}}
							</ul>
						</div>
					</script>
				</th>

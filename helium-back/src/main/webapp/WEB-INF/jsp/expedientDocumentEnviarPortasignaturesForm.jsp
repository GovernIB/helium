...
<body>
	<c:if test="${potFirmar}">
		<c:set var="formAction">
			<c:url value="/expedient/${expedientId}/proces/${document.processInstanceId}/document/${document.id}/enviarPortasignatures"/>
		</c:set>

		<form:form 	cssClass="form-horizontal content" action="${formAction}" enctype="multipart/form-data" method="post" modelAttribute="documentExpedientEnviarPortasignaturesCommand">
			<div>
		...
		function seleccionarCarrec(carrec) {
			...
			function recuperarFluxSeleccionat(portafirmesEnviarFluxId) {
				...
				if (portafirmesEnviarFluxId != null && portafirmesEnviarFluxId != '') {
					$.ajax({
						...
						url: "<c:url value="/modal/expedient/portafirmes/flux/mostrar"/>",
			...
			function crearFlux(documentNom) {
				$.ajax({
					...
					url: "<c:url value="/expedient/${expedientId}/proces/${document.processInstanceId}/document/${document.id}/portafirmesFlux/iniciarTransaccio"/>",
					success: function(transaccioResponse, textStatus, XmlHttpRequest) {
						if (transaccioResponse != null && !transaccioResponse.error) {
			...
			function obtenirPlantilles() {
				...
				$.ajax({
					type: 'GET',
					dataType: "json",
					url: "<c:url value="/expedient/${expedientId}/proces/${document.processInstanceId}/document/${documentExpedientEnviarPortasignaturesCommand.id}/flux/plantilles"/>",

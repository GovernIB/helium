...
<body>			
	...
	$(document).ready(function() {
		...
		$('#btnAddSortida').click(function() {
			...
			$.ajax({
			    url:"<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/sortida/add/"></c:url>" + estatSortidaId,
			    type:'POST',
			    dataType: 'json',
			    cache: false,
	...
	<fieldset>
		...
		<table	id="estatSortidaDataTable"
				...
				data-url="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/sortida/datatable"></c:url>"
			...
			<thead>
				<tr>
					...
					<th data-col-name="id" data-template="#cellEstatAccionsSortidaTemplate" data-orderable="false" width="10%">
						...
						<c:if test="${!heretat}">
					    	<a href="<c:url value="/expedientTipus/${expedientTipus.id}/estat/${estat.id}/sortida/"></c:url>{{:id}}/delete"  class="btn btn-danger" data-toggle="ajax" data-confirm="<spring:message code="expedient.tipus.estat.sortida.llistat.accio.esborrar.confirmacio"/>"><span class="fa fa-trash-o"></span></a>
						</c:if>
					</script>
					</th>
				</tr>

...
<c:set var="fluxosBaseUrl">
	<c:choose>
		<c:when test="${not empty expedientTipusDocumentCommand.definicioProcesId}">
			/definicioProces/${expedientTipusDocumentCommand.definicioProcesId}
		</c:when>
		<c:otherwise>
			/expedientTipus/${expedientTipusDocumentCommand.expedientTipusId}
		</c:otherwise>
	</c:choose>
</c:set>
...
<body>		
	<form:form cssClass="form-horizontal" action="${formAction}" enctype="multipart/form-data" method="post" modelAttribute="expedientTipusDocumentCommand">
		<div>			
			...
			<c:choose>
				<c:when test="${not empty expedientTipusDocumentCommand.definicioProcesId}">
					<c:set var="arxiuUrl">/definicioProces/${jbmpKey}/${expedientTipusDocumentCommand.definicioProcesId}/document/${expedientTipusDocumentCommand.id}/download</c:set>
				</c:when>
				<c:otherwise>
					<c:set var="arxiuUrl">/expedientTipus/${expedientTipusDocumentCommand.expedientTipusId}/document/${expedientTipusDocumentCommand.id}/download</c:set>
				</c:otherwise>
			</c:choose>
 			<hel:inputFile 
	 			name="arxiuContingut" 
						...
						<div class="col-xs-8">
							<p class="help-block" style="color: #8a6d3b">
								<span class="fa fa-exclamation-triangle"></span>
									<spring:message code="expedient.tipus.document.form.camp.id.flux.firma.innexistent" arguments="${expedientTipusDocumentCommand.portafirmesFluxId}, ${expedientTipusDocumentCommand.portafirmesFluxNom }"></spring:message>
							</p>
						</div>
					</div>
		
		
					<hel:inputHidden name="portafirmesFluxNom"></hel:inputHidden>			
				</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$.ajax({
					...
					success: function(data) {
								...
								if (plantillaActual != '') {
									...
									if (selPlantilles.val() != null) {
										$(".portafirmesFluxId_btn_edicio").attr("title", "<spring:message code="expedient.tipus.document.form.camp.portafirmes.flux.editar"/>");
										$('#avisFluxIdNoDisponible').hide();
									} else {
										// La plantila actual no està disponible mostra un avís
										$('#avisFluxIdNoDisponible').show();
									}
								}
					},
					error: function (error) {
			...
			$("#portafirmesFluxId").on('change', function () {
				...
				var data = $('#portafirmesFluxId').select2('data');
				if(data) {
					$('#portafirmesFluxNom').val(data.text);
				}
			});
		...
		function recuperarCarrecs() {
			...
			$.ajax({
				type: 'GET',
				dataType: "json",
				url: "<c:url value="/portasig/carrecs"/>",

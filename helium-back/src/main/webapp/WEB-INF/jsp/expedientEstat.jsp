...
//<![CDATA[
           $(document).ready(function(){
        	   	$('#btnDetall').click(function(){
        	   		...
        	   		$('#contingut-estats').load('<c:url value="/nodeco/expedient/${expedient.id}/estat?detall='+$('#detall').val()+'"/>');
        	   	});
							...
							<td>
								<c:choose>
									...
									<c:when test="${log.accioTipus == 'PROCES_SCRIPT_EXECUTAR'}">
										<a data-toggle="modal" href="<c:url value="/expedient/scriptForm/${log.id}"/>" class="icon a-modal-registre scriptLink_${log.id}"><i class="fa fa-search"></i></a>
									</c:when>

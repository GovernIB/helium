...
$(document).ready(function() {
	$('.proces').click(function() {
		...
		if ($(this).data('carrega') == "ajax") {
			...
			panell.load('<c:url value="/nodeco/expedient/${expedient.id}/proces/"/>' + id + '/dada', {"ambOcults": ambOcults}, updatePanell);
		}
	});
	$('#boto-ocults').click(function() {
		<c:choose>
			<c:when test="${ambOcults}">$('#contingut-dades').data('href', '<c:url value="/nodeco/expedient/${expedient.id}/dada"/>');</c:when>
			<c:otherwise>$('#contingut-dades').data('href', '<c:url value="/nodeco/expedient/${expedient.id}/dadaAmbOcults"/>');</c:otherwise>
		</c:choose>

...
function carregarInformacioOrgan() {
	...
	if ($('#organo').val() != '') {
		$.ajax({
			url:'<c:url value="/nodeco/expedientTipus/${expedientTipus.id}/comprovarOrgan/"/>' + $('#organo').val(),
		...
		<form:form cssClass="form-horizontal" enctype="multipart/form-data" method="post" modelAttribute="expedientTipusMetadadesNtiCommand">
			<div style="height: 400px">        
				...
				<div id="inputs_metadadesNti" style="display : ${not expedientTipusMetadadesNtiCommand.actiu ? 'none' : ''}">
					...
					<hel:inputText name="serieDocumental" textKey="expedient.tipus.metadades.nti.serie.documental" readonly="${not expedientTipusMetadadesNtiCommand.actiu}" required="true"/>

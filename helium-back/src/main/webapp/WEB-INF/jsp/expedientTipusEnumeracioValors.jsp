...
<c:choose>
  <c:when test="${expedientTipusId != null}">
  	<c:set var="baseUrl"><c:url value="/modal/expedientTipus/${expedientTipusId}/enumeracio/${enumeracio.id}/valor"></c:url></c:set>
  </c:when>
  <c:otherwise>
  	<c:set var="baseUrl"><c:url value="/modal/enumeracio/${enumeracio.id}/valor"></c:url></c:set>
  </c:otherwise>
</c:choose>
...
<body>			
	...
	<form:form id="validacio-form" cssClass="form-horizontal" action="#" enctype="multipart/form-data" method="post" modelAttribute="expedientTipusEnumeracioValorCommand" style='${mostraCreate || mostraUpdate ? "":"display:none;"}'>

...
<c:choose>
	<c:when test="${not empty expedientTipus}">
		...
		<form:form cssClass="form-horizontal" enctype="multipart/form-data" method="post" modelAttribute="expedientTipusIntegracioDistribucioCommand">
			<div>
					...
					<div class="col-xs-4">
						...
						<div id="reglaBtn" class="btn-group">
						  ...
						  <ul class="dropdown-menu">
						    <li>
								<a id="consultarReglaBtn" data-toggle="modal" href="<c:url value="/expedientTipus/${expedientTipus.id}/integracioDistribucio/consultarRegla?codiProcediment=${expedientTipusIntegracioDistribucioCommand.codiProcediment}"/>">
...
$(document).ready(function() {
	...
	$('#crearReglaBtn').click(function(e){
		...
		$(this).webutilNetejarErrorsCamps();
		var url = "<c:url value="/expedientTipus/${expedientTipus.id}/integracioDistribucio/crearRegla"></c:url>";
	    $.ajax({
	           type: "POST",
	           url: url,
	           data: {
	...
	$('#canviEstatReglaBtn').click(function(e){
		...
		$(this).webutilNetejarErrorsCamps();
		var url = "<c:url value="/expedientTipus/${expedientTipus.id}/integracioDistribucio/canviEstatRegla"></c:url>";
	    $.ajax({
	           type: "POST",
	           url: url,
	           data: {
	...
	$('#codiProcediment').change(function() {
		$('#consultarReglaBtn').attr('href', '<c:url value="/expedientTipus/${expedientTipus.id}/integracioDistribucio/consultarRegla?codiProcediment="/>' + $(this).val());
	})	
});
// ]]>
</script>			

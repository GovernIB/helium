...
public class TramitacioServiceSistraImpl implements TramitacioService {
	...
	private void crearZonaPers(String nif, String nom) throws es.caib.zonaper.ws.v2.services.BackofficeFacadeException, TramitacioException {
	...
	public RespostaAnotacioRegistre registrarNotificacio(RegistreNotificacio registreNotificacio) throws TramitacioException {
		...
		try {
			...
			if (registreNotificacio.getDadesInteressat() != null) {
				var datosInteresado = new DatosInteresado();
			...
			try {
				log.info("###===> Notificacio. Comprovar zona personal");
				...
				log.info("###===> Cridem al servei per registrar notificació");
				resultado = regtelClient.registroSalida(datosRegistroSalida);

				log.info("###===> S'ha cridat al servei correctament");
				...
				log.error("Error al registrar la notificacion electronica", ex);
			...
			log.error("Error al registrar la notificacion electronica", ex);
			throw new TramitacioException("Error al registrar la sortida", ex);
		}
	}

 ...
 * del tipus d'expedient en el cas d'entrada a l'estat per expedients basats en estats en comptes
 * de flux.
 * 
...
public class EstatAccioEntrada implements Serializable, GenericEntity<Long> {
    ...
    @ForeignKey(name="hel_estacc_ent_estat_fk")
    private Estat estat;

    @NotNull
    @ManyToOne(optional=false)
    @JoinColumn(name="accio_id")
    @ForeignKey(name="hel_estacc_ent_accio_fk")
    private Accio accio;
